	...
	function success(data, status, req) {
		...
		for (var i = 0; i < data.length; i++) {
			if(data[i].LOTAGE*1>=gMaxLotLifeMonths){
				...
				data[i].escalated=1;
				data[i].escalatedText="שים לב! הלוט באיסוף הוא לוט ישן. אנא פנה להבטחת איכות.";
			}
			//data[i].COUNTCO=data[i].COUNTCO*1;
			//data[i].RLDT=glob_convertAS400dateWithHyphen(data[i].OARLDT);
		}
		gMoPickData = data;
...
function getSelectedPickMoBano(bano) {
	...
	for (i = 0; i < gMoPickData.length; i++) {
		if (gMoPickData[i][fieldName] == bano) {
			ret = i;
			break;
		}
	}
