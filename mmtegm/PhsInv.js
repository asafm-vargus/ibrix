...
function buildSTNBlist(){
	var whlo = $("#inpWHLOPhsInvinputs").val();
...
function PhsInvEnter(enteredField) {
	...
	var whlo = $("#inpWHLOPhsInvinputs").val();
	var whsl = $("#inpWHSLPhsInvinputs").val().toUpperCase();
	$("#inpWHSLPhsInvinputs").val(whsl);
...
function PhsInvDo(){
	...
	if(gPhsLine!=false){
		if( trqt *1 != gPhsLine.SDSTQR*1){
			  $('body').message({
		          title: 'כמות חריגה',
		          message: 'כמות שהוקלדה שונה מכמות בספירה! <br /> האם להמשיך?',
		          returnFocus: $(this),
		          buttons: [{
		            text: 'כן',
...
function clearPhsScreen(){
	...
	$("#inpWHSLPhsInvinputs").val(whsl);
	if($("#inpWHSLPhsInvinputs").val()==""){ //30.7.20
		$("#inpWHSLPhsInvinputs").focus();
		$("#inpWHSLPhsInvinputs").select();
	}else{
