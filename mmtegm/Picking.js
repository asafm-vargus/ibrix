...
function getPickData(enteredField) {
	...
	if (enteredField == "inpSmallBoxBARCODE" || enteredField == "inpSmallBoxTRQT") {

		if (bano.substring(3,4) == "-"){	// ITNO
			var mms200 = runAPI_MMS200MI_GetItmBasic(bano);
			if(mms200!="")
				gSmallBoxBanoData = {LMITNO:bano, MBTOMU:"", MMITDS:mms200.ITDS};
			else
				return;
			bano = "";
		}
		...
		if (gSmallBoxBanoData.LMITNO != undefined) {
			...
			/*	
				...
				*/
				...
				inputFields.TTYP = gPickListData.OQTTYP;
		...
		//	var packed=(currentPacked * 1) + (trqt * 1);
			...
			trqt = $("#inpSmallBoxTRQT").val()*1; // ASAF 20210805
			if (trqt == "" || trqt == 0){
				$("#inpSmallBoxTRQT").focus(); //12.7.21
				$("#inpSmallBoxTRQT").select(); //12.7.21
			}else{
...
function BigBoxDetailEnter(enteredField) {
	...
	if (enteredField == "inpBigBoxBARCODE" || enteredField == "inpBigBoxITNO") {
		var panr="9999";

		gBigBoxBanoData= getQnatityFromMftrns(panr,bano, whlo, dlix, itno);
		if(gBigBoxBanoData==false){
			gBigBoxBanoData= getQnatityFromMftrns("",bano, whlo, dlix, itno); //no 9999 panr
		}
...
function getQnatityFromMftrns(panr,bano, whlo, dlix, itno) {

	var ret = {};

	var WherePanr="";
	if(panr!="") 
		WherePanr=" and OSPANR='" + panr + "' ";
	if (itno != "") 
		WherePanr += " and OSITNO='" + itno + "' ";
	var dataToSend = {
		usepool : 1,
