...
function getPoData(enteredField){
	...
	if(enteredField=="inpQA"){
		...
		$("#inpWHSLPoRinputs").val(gotWHSL);
		$("#inpWHSLPoRinputs").focus();
		
	}
	
	if(enteredField=="inpWHSLPoRinputs"){
		pps001do();
...
function pps001do(isSPok){
	...
	var whsl= $("#inpWHSLPoRinputs").val();
	if(whsl==""){
		 topSoHo.alert("איתור לא חוקי!");
		 return;
	}
	...
	if(isSPok==false){
		...
		checkSP($("#inpWHLOPoRinputs").val(),$("#inpITNO").val(),'P','R',$("#inpQA").val(),$("#inpPUNO").val(), gPurchaseOrderLineDetails.IBPNLI,whsl);	
     ...
     obj_Row.WHSL =whsl;
   //  obj_Row.BANO = bano;
     obj_Row.WHLO = $("#inpWHLOPoRinputs").val();
...
function RunPPS001MI_Receipt(inputFields) {
   ...
   $('body').toast({
		title : 'קליטה מרכש',
		message : 'הזמנה:' + inputFields.PUNO   + ' כמות :' + inputFields.RVQA 
	});
   return true;
}
