...
var gMaxLotLifeMonths=36; // 25.5.21 for EGMO 3 years
						...
						function() {
							if (logonwin == undefined) {
								...
								$('body')
										.message(
												{
													title : '<span>שגיאה</span>',
													status : 'error',
													returnFocus : $(this),
													message : 'יש להקליד שם וסיסמה. ייתכן וחלונות קופצים נחסמו. יש להוסיף את האתר לאתרים המורשים לחלונות קופצים',
													buttons : [ {
														text : 'נסה שוב',
														click : function() {
															login();
...
function showPickingMenuDO(clearDLIX){
	...
	$("#headerTitle").html("תפריט איסוף");
	$(".divsection").hide(); // hide all
	$("#maincontentPickingMenu").show();
	
	$("#inpWHLO.Pickinputs").val(glob_ObjM3UserProp.WHLO);
	//buildPACTlist();
	if(clearDLIX==true) {
		$("#inpDLIX").val("");
		$("#inpCONA").html("");
	...
	$("#inpDLIX").focus();
	if($("#inpDLIX").val()!="") getPickData("inpDLIX");
}
...
function showSmallBox(){
	...
	var ttl="Small Box משלוח:" + gPickListData.PIDLIX + "/" + gPickListData.PIPLSX;
...
//	$("#btnPickingBack").show();
	...
	var ttl="Big Box משלוח:" + gPickListData.PIDLIX + "/" + gPickListData.PIPLSX;
	$("#headerTitle").html(ttl);
...
//	$('#maincontentMainMenu').trigger('start.busyindicator');
	...
	$("#headerTitle").html("העברה בין איתורים");
...
function showMoReport() {
	...
	$("#headerTitle").html("דיווח ייצור");
	$("#inpWHLOMoRinputs").val(glob_ObjM3UserProp.WHLO);
...
function runAPI_MMS200MI_GetItmBasic(itno) {
	...
	var returncols = 'ITNO,ITDS,GRTI,UNMS,STAT,ITTY,FCU1';
...
//	$(".divsection:not(#maincontentBigBox)").hide(); // hide all except maincontentBigBox
	...
	var ttl="ר.איסוף:" + gCurrentMoPickDelivery.OQDLIX + "/" + gCurrentMoPickDelivery.PIPLSX;
	$("#headerTitle").html(ttl);
...
function showPhsInv(){
	...
	$("#headerTitle").html("ספירת מלאי");
	$(".PhsInvinputs").val("");
	$("#inpWHLOPhsInvinputs").val(glob_ObjM3UserProp.WHLO);
