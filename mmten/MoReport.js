...
function getMoData(enteredField){
	...
	var whlo=$("#inpWHLO.MoRinputs").val();
	
	if(enteredField=="inpMFNO"){
	//	$("#inpMoREQA").prop("readonly",false); //12.2.20
		var mfno=$("#inpMFNO").val();
		var whlo=$("#inpWHLO.MoRinputs").val();
	...
	if(whlo=="101" && enteredField=="txtPARTqty" ){
		if($("#txtPARTqty").val()>0) {
			$("#inpMoPartWHSL").val(getDefaultWHSL($("#inpWHLO.MoRinputs").val(),gMoDetails.VHPRNO,'M','R',$("#txtPARTqty").val(),gMoDetails.VHMFNO+'P' ).DFT_LOC);
			$("#inpMoWHSL").focus();
			$("#inpMoWHSL").select();
		}
	}
...
function getMoDetails(whlo,mfno){
	...
	function success(data, status, req) {
		if (status == "success" && data.length > 0) {
			...
			topSoHo.alert("Manufacturing order not found!");
			ret=false;
		}
...
function calculatePartialAndPerform(){ //13.9.18
	...
	$("#inpMoWHSL").val(getDefaultWHSL($("#inpWHLO.MoRinputs").val(),gMoDetails.VHPRNO,'M','R',txtFULLQty,gMoDetails.VHMFNO ).DFT_LOC);
...
function pms050do(isSPok){
	...
	if(whlo==undefined || whlo==""){
		...
		$("#inpWHLO.MoRinputs").focus();
		$("#inpWHLO.MoRinputs").select();
		return;
	}
	...
	if(mfno==undefined || mfno==""){
		topSoHo.alert("Manufacturing order must be entered!");
		$("#inpMFNO").focus();
		$("#inpMFNO").select();
		return;
	}
	...
	*/
	...
	if(isSPok==false){
		...
		checkSP($("#inpWHLO.MoRinputs").val(),gMoDetails.VHPRNO,'M','R',$("#inpMoREQA").val(),gMoDetails.VHMFNO,0,$("#inpMoWHSL").val() );	
...
function RunPMS050MI_RptReceipt(inputFields) {
   ...
   $('body').toast({
		title : 'MO Report',
		message : 'MO:' + inputFields.MFNO   + ' Quantity :' + inputFields.RPQA 
	});
   return true;
}
