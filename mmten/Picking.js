...
function getPickData(enteredField) {
	...
	if (enteredField == "inpDLIX") {
		...
		var dlix = $("#inpDLIX.Pickinputs").val();
		$("#bdgSmallBox").text("");
		$("#bdgBigBox").text("");
		...
		showPackageList();
		 ...
		 $("#inpDLIX.Pickinputs").focus(); //7.4.21 vne wants focus in DLIX
		 $("#inpDLIX.Pickinputs").select(); //7.4.21 vne wants focus in DLIX 
	}
	...
	if (enteredField == "inpSmallBoxBARCODE") {
		...
		if (gSmallBoxBanoData.LMITNO != undefined) {
			...
			if(trqt>totalAvailableQty){ //11.12.19
				topSoHo.alert("Quantity larger than remain quantity!");
				return false;
			}
		...
		//	var packed=(currentPacked * 1) + (trqt * 1);
			...
			if( $("#inpSmallBoxTRQT").val()*1>0) {
				refreshSmallBoxScreen(); //7.4.21 was always clearing screen
			}else{ //7.4.21
				$("#inpSmallBoxTRQT").focus();
				$("#inpSmallBoxTRQT").select()	
			}
	...
	if (enteredField == "inpSmallBoxTRQT") { //7.4.21
		let trqt = $("#inpSmallBoxTRQT").val()*1; //12.4.20
		if(trqt>0){
			getPickData("inpSmallBoxBARCODE");
		}
...
function refreshSmallBoxScreen() {
	...
	$("#inpWHSL.SmallBoxinputs").focus(); //7.4.21
	$("#inpWHSL.SmallBoxinputs").select(); //7.4.21
...
function searchPickingLists(dlix, whlo) {
	...
	function success(data, status, req) {
		if (status == "success" && data.length > 0) {
			...
			BadScanAudio.play();
			topSoHo.alert("Delivery not found !");
...
function getPickMitaloData(dlix, plsx, bano, whlo, whsl, itno) {
	...
	function success(data, status, req) {
		if (status == "success" && data.length > 0) {
			...
			topSoHo.alert("Lot "  + bano + " not found!");
			ret = false;
		}
...
function runAPI_MWS423MI_PackLine(inputFields) {
	...
	$('body').toast(
			{
				title : 'Pick Small Box',
				message : 'Lot:' + inputFields.BANO + ' Quantity :'
						+ inputFields.PAQT + ' to Box:' + inputFields.PANR
...
function runAPI_MWS423MI_DltPackage(inputFields) {
	...
	$('body').toast(
			{
				title : 'Pick Small Box',
				message : 'Box:' + inputFields.PANR + ' unpacked!'
				,position:'top right',timeout:3000
...
function runAPI_MWS423MI_ConnectPackages(inputFields) {
	...
	$('body').toast(
			{
				title : 'Pick Big Box',
				message : 'Box:' + inputFields.PANR + " packed inside: " + inputFields.PAII
				,position:'top right',timeout:3000
			});
	// GoodScanAudio.play();
	return true;// anserAtt.ITNO.trim();
...
function runAPI_MWS423MI_ChangePackage(inputFields) {
	...
	$('body').toast(
			{
				title : 'Pick Big Box',
				message : 'Box:' + inputFields.PANR + ' Weight:' +  inputFields.GWTM
				,position:'top right',timeout:3000
			});
	// GoodScanAudio.play();
	return true;// anserAtt.ITNO.trim();
...
function delSmallPanr(panr){
	  ...
	  $('body').message({
          title: 'Unpack',
          message: 'Are you sure you want to unpack box ' + panr,
          returnFocus: $(this),
          buttons: [{
            text: 'Yes',
	...
	if (enteredField == "inpBigBoxTRQT") {
		...
		if(bano!="" && trqt>0){
			...
			if(panr=="9999" && trqt*1> gBigBoxBanoData.OSDLQT*1){
				topSoHo.alert("Can't pack more than Small Box content");
	...
	function success(data, status, req) {
		if (status == "success" && data.length > 0) {
			...
			if(paii!=undefined && paii.trim()!=""){
				topSoHo.alert("Box " + panr + " already packed inside " + data[0].ORPAII);
				ret=true;
			}
			// $(".pickMenuButtons").attr("disabled",false);
		
		}else{
			topSoHo.alert("Box " + panr + " doesn't exists! ");
			ret=true;
		}
...
function showPackageContent(panr) {
	...
	$("#crdListLotsTitle").html("Box content " + panr);
...
function showPackageListByItem() {
	...
	function success(data, status, req) {
		...
		//"OSITNO":  "021-52033","OSBANO":  "1001459859P","OSRIDN":  "0001439972","OSRIDL":  "4","OSPANR":  "2","OSDLIX":  "3526766","OSCONO":  "1","OSWHLO":  "101","MQWHSL":  "PACKCON","OSDLQT":  "1.0","OSCHID":  "intentia" },
...
function delSmallLot(id){
	   ...
	   var inputFields = {};
		...
		var msg='Are you sure you want to unpack Lot ' + inputFields.BANO + " from Box " + gSmallBoxListPackedItems[id].OSPANR;
		if(gSmallBoxListPackedItems[id].OTHERFLAG!=undefined &&  gSmallBoxListPackedItems[id].OTHERFLAG!="") msg+="<br /><b>Notice!" + gSmallBoxListPackedItems[id].OTHERFLAG + "</b>";
		var res=true;
