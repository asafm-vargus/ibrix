...
function glob_getCurrentDateTime() {
    ...
    objDateTime.weekOfYeare = Math.ceil(dayOfYeare / 7);
    if (objDateTime.weekOfYeare)

    objDateTime.m3Date = objDateTime.year + "" + objDateTime.month + "" + objDateTime.day;
    objDateTime.m3Time = objDateTime.hours + "" + objDateTime.minutes + "" + objDateTime.sec;
    objDateTime.m3Week = (objDateTime.year * 100 + objDateTime.weekOfYeare);

    var tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
    objDateTime.tomorrow = {
        year: tomorrow.getFullYear(),
        yearSm: tomorrow.getYear() - 100,
        month: tomorrow.getMonth() + 1,
        day: tomorrow.getDate()
    };
    if (objDateTime.tomorrow.month < 10) objDateTime.tomorrow.month = "0" + objDateTime.tomorrow.month;
    if (objDateTime.tomorrow.day < 10) objDateTime.tomorrow.day = "0" + objDateTime.tomorrow.day;

    var yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    objDateTime.yesterday = {
        year: yesterday.getFullYear(),
        yearSm: yesterday.getYear() - 100,
        month: yesterday.getMonth() + 1,
        day: yesterday.getDate()
    };
    if (objDateTime.yesterday.month < 10) objDateTime.yesterday.month = "0" + objDateTime.yesterday.month;
    if (objDateTime.yesterday.day < 10) objDateTime.yesterday.day = "0" + objDateTime.yesterday.day;
...
function glob_SQL_getEmployee(emno) {
    ...
    var url = "Queries/getEmploee.jsp?cono=" + glob_ObjM3UserProp.CONO + "&faci=" + glob_ObjM3UserProp.FACI + "&emno=" + emno;
    console.log(url)
    $.ajax({
        cache: false,
        async: false,
...
function glob_convertYYYYMMDDtodisplay(DDMMYY)  //yyyymmdd to dd/mm/yyyy
{
    ...
    dd = DDMMYY.substr(6, 2);
    mm = DDMMYY.substr(4, 2);
    yy = DDMMYY.substr(0, 4);

    var dateDisplay = dd + "/" + mm + "/" + yy;
    return dateDisplay;
...
function glob_setSimpelDateFromM3Date(m3date, sup) {
    m3date=m3date.toString();
    return m3date.substr(6, 2) + sup + m3date.substr(4, 2) + sup + m3date.substr(0, 4);
}
...
function glob_ConvertDatetoM3(dt) //18.4.16
{
    return glob_ConvertDatetoM3_DTFM(dt, glob_ObjM3UserProp.DTFM);
}

function glob_ConvertDatetoM3_DTFM(dt, dftm) //7.2.16
{
...
function glob_convertM3TimeToSigleTime(M3Time, milisec) { //ddmmyy to yyyymmdd
    if (isNaN(parseInt(M3Time)) || M3Time.length < 5 || M3Time.length > 6) {
        return null;
    }
    ...
    if (milisec == undefined || milisec == null || !milisec) {
        return M3Time.substr(0, 2) + ":" + M3Time.substr(2, 2);
    } else {
        return M3Time.substr(0, 2) + ":" + M3Time.substr(2, 2) + ":" + M3Time.substr(4, 2);
    }
}
...
/***
 * cookie functions
 * must insert in html: <script src="../sors/jquery.cookie.js"></script>
 * setCooke() to create new cookie, if data exist -  overwriting data
 * getCooke() get data from exist cookie, if data not exist return null
 * @param par - name fo saving data
 * @param val - value to save
 */
function glob_setCooke(par, val) {
    $.cookie(par, val);
}
function glob_getCooke(par) {
    var retVal = $.cookie(par);
...
/**
 * validate input to numbers with decimal
 * @param evt - event of pres
 */
function global_validateNumWithDec(evt) {
...
function glob_DisplayCurrentTimeOnly() {
    var dt = new Date();
    var refresh = 1000; //Refresh rate 1000 milli sec means 1 sec
