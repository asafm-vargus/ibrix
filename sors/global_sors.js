...
var glob_WsUrl = "http://cs-m3home.corp.aipm.co.il:30007/mws-ws/svc/";


var glob_launchTaskWindow;
function glob_launchTask(uri){ /*08.4.19*/
	if (typeof parent.launchTask === "function") { 
	    // safe to use the function
		parent.launchTask(uri); //use H5 function
	}else{
		uri=uri.replace("_automation/?data","");
		var xml=uri;//encodeURIComponent(uri);		
		 var url = "../sors/launchSmartOffice.jsp?xml="+xml;
		 glob_launchTaskWindow=window.open(url,"","width=200,height=100");
		 /*please note that chrome's behaviour of "site cannot be reached" may launch SO mulitiple times!*/
		 window.setTimeout(function(){ 
			 glob_launchTaskWindow.close(); 
		 },1000)
...
/*21.1.19*/
function glob_IsProduction() {
    var port = window.location.port;
    var ret = false;
    if (port == 19007 || port == 19008 || port == 30010 || port == 19010 || port == 16101 || port == 16102 || port == 16111) ret = true;
...
/*21.1.19*/
function glob_getDBschema() {

    var ret = "M3FDBPRD";
    if (!glob_IsProduction) ret = "M3FDBTST";
...
/*26.12.18 Jonathan legacy ibrix top.alert to new SoHo message */
var topSoHo = {};

topSoHo.alert = function (msg, ttl) {
    if (ttl == undefined) ttl = msg;
...
topSoHo.complete = function (msg, ttl) { /*2.4.19*/
    if (ttl == undefined) ttl = msg;
...
topSoHo.success = function (msg, ttl) { /*2.4.19*/
    if (ttl == undefined) ttl = msg;
 ...
 *
 */
function glob_DisplayCurrentTime(dateHTMLtag) {
    var dt = new Date();
 ...
 *
 */
function glob_RunApi_OneAnswer(program, transaction, returncols, inputFields,withTrim) { //3.4.19 jonathan added trim support
...
function glob_RunApi_OneAnswer_POST(program, transaction, returncols, inputFields, withTrim) {
    // var ansers= new Array();
	if(withTrim==undefined) withTrim=false; //10.3.19
    var anserAtt = {};
    // return OBJ
    ...
    function processSuccess(data, status, req) {
        ...
        if (status == "success") {
            ...
            if ((req.responseText).indexOf("ErrorMessage") > -1) {
                ...
                var mesErr = ApiError + ": " + txt_Messege.trim();
                console.log("error from Api=" + ApiError);
                anserAtt.OnlyError=txt_Messege.trim(); //31.3.19
                anserAtt.error = mesErr;
...
function glob_RunApi_MultiAnswer_POST(program, transaction, returncols, inputFields,withTrim) {//dennis 27/11/16
    // var ansers= new Array();
	if(withTrim==undefined) withTrim=false; //10.3.19
    ...
    var url = '../../m3api-rest/execute/' + program + '/' + transaction + ';maxrecs=' + maxrecs + ';returncols=' + returncols;
    if (returncols == "") {
        url = '../../m3api-rest/execute/' + program + '/' + transaction + ';maxrecs=' + maxrecs + ';';
    }
    ...
    function processSuccess(data, status, req) {
        ...
        if (status == "success") {
            ...
            if ((req.responseText).indexOf("ErrorMessage") > -1) {
                ...
                var MIRecord = $(req.responseText).find('MIRecord');
                anserAtt.rows = [];
                for (var i = 0; i < MIRecord.length; i++) {
                    var NameValue = $(MIRecord[i]).find('NameValue');
                    var val1 = $(NameValue).find('Value');
                    var line = {};
                    $(val1).each(function (index, ell) {
                        var nm = NameValue[index].firstChild.childNodes[0].data;
                        if(withTrim==true) nm=nm.trim(); //10.3.19
                        var vl = ell.firstChild.data;
                        if(withTrim==true) vl=vl.trim(); //10.3.19
                        line[nm] = vl;
                    });
                    anserAtt.rows.push(line);
                }
            }
        }
    }
...
function glob_API_getCurrentUser() {
    ...
    var inputFields;
    var retObj = glob_RunApi_OneAnswer(program, transaction, returncols, inputFields,true); //20.5.19 add true for teim 
...
function glob_GetServerTime() {
    ...
    var inputFields = "";
    return glob_RunApi_OneAnswer(program, transaction, returncols, inputFields,true); //10.3.19 added true for trim
}
...
function glob_convertTime(tm){ //HHMM to HH:MM //10.4.19 
	tm=glob_AddLeadZero(tm,4);
	var hh=tm.substring(0,2);
	var mm=tm.substring(2,4);
	return hh + ":" + mm;
...
function glob_timeFloatToString(time) {
	
	  var hoursMinutes = time.split(/[.]/);
	  var hours = hoursMinutes[0];
	  var mins=hoursMinutes[1] ;//22.10.15
	  if(mins==undefined || mins=="") mins=0;
	  var minutes = "0." + mins;
	  return hours + ":" + glob_AddLeadZero(60*minutes,2) ;
	}


function glob_convertTime6(tm){ //HHMM to HH:MM //10.4.19 
	tm=glob_AddLeadZero(tm,6);
	var hh=tm.substring(0,2);
	var mm=tm.substring(2,4);
	return hh + ":" + mm;
...
function glob_convertToAS400dateNoSlash(dt)// ddmmyy to 20yymmdd //10.3.19
{
	var yy="20" + dt.substring(4,8);
...
function glob_convertToAS400dateNoSlashWithHyphen(dt)// dd-mm-yy to 20yymmdd
{
    var yy = "20" + dt.substring(6, 8);
...
function glob_convertAS400dateWithHyphen(dt) //3.1.19  YYYYMMDD to DD-MM-YY
{
	dt=dt+""; //13.5.19 convert to string 
...
function glob_convertDateWithHyphen(dt) //28.4.19  DDMMYY to DD-MM-YY
{
    var dd = dt.substring(0, 2);
    var mm = dt.substring(2, 4);
    var yy = dt.substring(4, 6);
...
function glob_convertAS400date(dt) //1.3.19 YYYYMMDD to DDMMYY
{
    var yy = "" + dt.substring(2, 4);
    var mm = dt.substring(4, 6);
    var dd = dt.substring(6, 8);
...
function glob_convertAS400dateWithSlash(dt) //21.1.19  YYYYMMDD to DD/MM/YY
{
    var yy = "" + dt.substring(2, 4);
    var mm = dt.substring(4, 6);
    var dd = dt.substring(6, 8);
...
function glob_convertDateObjextToYYYYMMDD(date) { //20.3.19 https://stackoverflow.com/questions/23593052/format-javascript-date-to-yyyy-mm-dd
    var d = new Date(date), //must be in format MM/DD/YYYY
        month = '' + (d.getMonth() + 1),
        day = '' + d.getDate(),
        year = d.getFullYear();
...
function glob_convertDDMMYYYYWithSlashtoYYYYMMDD(dt)  //dd/mm/yyyy to yyyymmdd
{
